    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <button class="mt-4 text-white bg-emerald-700 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800" onclick="location.href='{{ route('mobil.create') }}'" type="button">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tr class="odd:bg-white even:bg-slate-50 dark:even:bg-gray-800 dark:odd:bg-gray-700 dark:text-white">
                            <td class="p-3 text-left text-sm whitespace-nowrap">{{ $data->pembelian->mobil->merek }} {{ $data->pembelian->mobil->tipe }}</td>
                            {{-- Jika admin maka menampilkan akun pembeli --}}
                            @role('sales')
                            <td class="p-3 text-left text-sm whitespace-nowrap">{{ $data->pembelian->user->name }}</td>
                            @endrole('sales')
                            <td class="p-3 text-left text-sm whitespace-nowrap">{{ $data->pembelian->nama_penerima }}</td>
                            <td class="p-3 text-left text-sm whitespace-nowrap">{{ $data->pembelian->nomor_penerima }}</td>
                            <td class="p-3 text-left text-sm">{{ $data->pembelian->alamat_penerima }}</td>
                            <td class="p-3 text-left text-sm whitespace-nowrap">{{ $data->pembelian->jumlah }}</td>
                            <td class="p-3 text-left text-sm ">{{ $data->pembelian->status }}</td>
                            <td class="p-3 text-left text-sm whitespace-nowrap">Rp. {{ number_format($data->pembelian->harga_total, 0, ',', '.') }}</td>
                            <td class="p-3 text-left text-sm whitespace-nowrap">
                                <div class="flex item-center w-auto">
                                    <a href="{{ route('pembelian.cetakInvoice', $data->pembelian->id) }}" class="hover:text-gray-100  mr-2">
